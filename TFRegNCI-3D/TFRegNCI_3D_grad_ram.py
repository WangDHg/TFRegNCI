...
from TFRegNCI_3D_model import loss_func, merge_model
import os
import math
import sys

import argparse
from grad_ram import GradRAM
...
def train(train_loader, train_data, train_cube, train_labels_n, optimizer, net, scaler, epoch, lr, epoch_size):
    ...
    for index, items in enumerate(train_loader):
        ...
        cube_train = torch.tensor(train_cube[items]).cuda(0).reshape(-1,1,137,133,124).float()
...
def evaluate(data_loader, chem, cube, labels, bzhcs, net):
    ...
    for index, items in enumerate(data_loader):
        ...
        cube_test = torch.tensor(cube[items]).cuda(0).reshape(-1,1,137,133,124).float()
...
def main(seed):
    ...
    for epoch in range(epoch_size):
        ...
        data_index = 1
        data_index -= 1
        input_tensor = torch.from_numpy(test_cube[data_index]).reshape(-1,1,137,133,124)
        chem_input = torch.from_numpy(test_data[data_index]).reshape(-1,21)
        net.eval()

        targets = train_labels_n[data_index]
        
        model = net
        target_layers = [model.cube_conv.trunk_output[-1]]

        cam_algorithm = GradRAM
        with cam_algorithm(model=model,
                        target_layers=target_layers,
                        use_cuda=args.use_cuda) as cam:

            cam.batch_size = 50
            grayscale_cam = cam(input_tensor=input_tensor,chem_input=chem_input, targets=targets, aug_smooth=args.aug_smooth, eigen_smooth=args.eigen_smooth)
        ...
        location_map = np.reshape(cam_image,(-1, 1))
        np.savetxt(output_path+f'{epoch}.cube', location_map, fmt='%e', delimiter='   ')
